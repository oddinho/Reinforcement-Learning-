...
from train_target_selection_ppo_v5 import (
    DEFAULT_CHECKPOINT,
    DEFAULT_POSITION_AWARE_PPO_CHECKPOINT,
    DEFAULT_PPO_CHECKPOINT,
    build_ppo_checkpoint_opponent,
    load_checkpoint,
)
...
DEFAULT_OUTPUT = EXPERIMENT_ROOT / "recordings" / "target_selection_ppo_v5_vs_bfs_seed0.json"


def build_opponent(name, seed, device, args):
    if name == "bfs":
        ...
        return build_ppo_checkpoint_opponent(args.ppo_checkpoint, seed, device)
    elif name == "position_aware_ppo":
        return build_ppo_checkpoint_opponent(
...
def main():
    ...
    opponent = build_opponent(args.opponent, args.seed, device, args)
