...
ROUTE_DEPTH = 3
ROUTE_DECAY = 0.7
DEFAULT_LOCAL_CLUSTER_RADIUS = 3
DEFAULT_LARGE_CLUSTER_RADIUS = 5
...
def build_target_candidates(
    observation,
    local_cluster_radius=DEFAULT_LOCAL_CLUSTER_RADIUS,
    large_cluster_radius=DEFAULT_LARGE_CLUSTER_RADIUS,
):
    ...
    for item in items:
        ...
        cluster3 = count_cluster(
            item_distance_maps,
            item,
            items,
            radius=local_cluster_radius,
        )
        cluster5 = count_cluster(
            item_distance_maps,
            item,
            items,
            radius=large_cluster_radius,
        )
        ...
        (
            ...
            contested=contested,
            cluster_radius=large_cluster_radius,
        )
...
class TargetSelectionMLP(nn.Module):
    ...
    the same small feature vector and receives one logit. The v5 actor appends
    the opponent's absolute board position to the v3 target features. The critic
    receives a pooled summary of the current candidate set.
    """
...
class DeterministicTargetSelectionAgent:
    ...
    def __init__(
        ...
        self.local_cluster_radius = int(local_cluster_radius)
        self.large_cluster_radius = int(large_cluster_radius)

    def act(self, observation):
        candidates = build_target_candidates(
            observation,
            local_cluster_radius=self.local_cluster_radius,
            large_cluster_radius=self.large_cluster_radius,
        )
