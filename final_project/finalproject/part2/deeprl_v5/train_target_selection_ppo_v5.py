...
from target_selection_ppo_mlp_v5 import (
    ACTION_DIM,
    FEATURE_DIM,
    GLOBAL_FEATURE_DIM,
...
DEFAULT_CHECKPOINT = CHECKPOINT_DIR / "target_selection_ppo_mlp_v5_latest.pt"
...
def collect_rollout(model, args, device, rng, iteration):
    ...
    for _step in range(args.rollout_steps):
        ...
        candidates = build_target_candidates(
            obs["player_0"],
            local_cluster_radius=args.local_cluster_radius,
            large_cluster_radius=args.large_cluster_radius,
        )
        if candidates is None:
...
def live_evaluate(model, device, opponent_name, seed, max_steps, args=None):
    env = CollectorGymEnv(numpy_output=True)
    local_cluster_radius = getattr(args, "local_cluster_radius", 3)
    large_cluster_radius = getattr(args, "large_cluster_radius", 5)
    agent = DeterministicTargetSelectionAgent(
        model,
        device,
        seed=seed,
        local_cluster_radius=local_cluster_radius,
        large_cluster_radius=large_cluster_radius,
    )
    ...
    torch.save(
        {
            ...
            "architecture": "target_selection_mlp_tanh_2x64_cluster_radius_sweep_v5",
        },
        path,
    )
...
def adapt_checkpoint_state_dict(model, checkpoint, state_dict):
    """Load older checkpoints into the widened 33-feature v5 model.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Used by the league loop to make newer accepted snapshots "
    ...
    parser.add_argument("--local-cluster-radius", type=int, default=3)
    parser.add_argument("--large-cluster-radius", type=int, default=5)
    parser.add_argument(
        "--reward-mode",
...
def main():
    ...
    if args.eval_only:
        print(
            ...
            f"local_radius={args.local_cluster_radius} "
            f"large_radius={args.large_cluster_radius}",
            flush=True,
        )
